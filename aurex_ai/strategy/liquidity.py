...
def analyze(
        ...
        for c in event_zone:
            if c.high > level and c.close < level:
                ...
                if wr >= min_wick_ratio:
                    ...
                    log.debug(
                        "sweep: equal_highs_swept level=%.5f touches=%d wr=%.2f score=%.1f",
                        level, touches, wr, score,
                    )
                    return SweepResult(
                        ...
                        touches=touches,
                        reason=f"equal_highs level={level:.5f} touches={touches} wr={wr:.2f}",
                    )
        ...
        for c in event_zone:
            if c.low < level and c.close > level:
                ...
                if wr >= min_wick_ratio:
                    score = min(20.0, 12.0 + touches * 2.0 + wr * 4.0)
                    log.debug(
                        "sweep: equal_lows_swept level=%.5f touches=%d wr=%.2f score=%.1f",
                        level, touches, wr, score,
                    )
                    return SweepResult(
                        ...
                        touches=touches,
                        reason=f"equal_lows level={level:.5f} touches={touches} wr={wr:.2f}",
                    )
    ...
    if sh:
        ...
        for c in event_zone:
            if c.high > last_sh and c.close < last_sh:
                ...
                log.debug("sweep: swing_high_swept level=%.5f wr=%.2f", last_sh, wr)
                return SweepResult(
                    detected=True, sweep_type="sell_side",
    ...
    if sl:
        ...
        for c in event_zone:
            if c.low < last_sl and c.close > last_sl:
                wr    = _lower_wick_ratio(c)
                score = min(14.0, 8.0 + wr * 6.0)
                log.debug("sweep: swing_low_swept level=%.5f wr=%.2f", last_sl, wr)
                return SweepResult(
                    detected=True, sweep_type="buy_side",
    ...
    if uwr >= 0.65 and last.close < last.open:
    ...
    if lwr >= 0.65 and last.close > last.open:
